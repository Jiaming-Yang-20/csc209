...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    Image *test_image;
    unsigned char actual_label;
    unsigned char knn_label;
    for(int i = start_idx; i <= start_idx + N - 1; i++){
        test_image = &(testing->images[i]);
        actual_label = testing->labels[i];
        knn_label = knn_predict(training, test_image, K, fptr);
        if(actual_label == knn_label){
            num_correct += 1;
        }  
    }
