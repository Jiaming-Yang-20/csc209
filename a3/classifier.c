...
int main(int argc, char *argv[]) {
    ...
    // TODO
    int fdwrite[num_procs][2]; // pipes that distribute data to children
    int fdread[num_procs][2]; // pipes that get data from children
    for (int i = 0; i < num_procs; i++){
            if(pipe(fdwrite[i]) == -1){ // create a write pipe with each children
                perror("pipe");
                exit(1);
            }
            if(pipe(fdread[i]) == -1){ //  create a read pipe with each children
                perror("pipe");
                exit(1);
            }
            int res = fork();
            if(res < 0){
                ...
                if(close(fdwrite[i][1]) == -1){ // close ports that won't be used by children
                    perror("close"); //     since close is a syscall, I did error checking
                    exit(1);
                }
                if(close(fdread[i][0]) == -1){ // close ports that won't be used by children
                    perror("close");
                    exit(1);
                }

                for(int j = 0; j < i; j++){
                    if(close(fdread[j][0]) == -1){ // close read ports left over by the parent
                        perror("close");
                        exit(1);
                    }
                }
                child_handler(training, testing, K,fptr,fdwrite[i][0], fdread[i][1]);
                exit(0);
            }else{ // parent
                if(close(fdwrite[i][0]) == -1){// close ports that won't be used by parent
                    perror("close");
                        exit(1);
                }
                if(close(fdread[i][1]) == -1){// close ports that won't be used by parent
                    perror("close");
                        exit(1);
                }
            }
    ...
    // TODO
    int start_idx = 0;
    int C = test_set_size / num_procs + 1; // take the ceiling
    int F = test_set_size / num_procs; // take the floor
    int N;
    for (int i = 0; i < num_procs - 1; i++){ // for the first num_procs - 1 processes
        if(i % 2 == 0){
             N = C;
        }else{
             N = F;
        }
        if(write(fdwrite[i][1], &start_idx, sizeof(int)) == -1){ // write start_idx
            perror("write");
            exit(1);
        }
        if(write(fdwrite[i][1], &N, sizeof(int)) == -1){ // write N
            perror("write");
            exit(1);
        }   
        start_idx += N;
    ...
    int last_N = test_set_size - start_idx;
    if(write(fdwrite[num_procs - 1][1], &start_idx, sizeof(int)) == -1){
        perror("write");
        exit(1);
    }
    if(write(fdwrite[num_procs - 1][1], &last_N, sizeof(int)) == -1){
        perror("write");
        exit(1);
    }

    for (int i = 0; i < num_procs; i++){ // after finish write, close all the write ports
            if (close(fdwrite[i][1]) == -1){
                perror("close");
                exit(1);
            }
    }
    ...
    for(int i = 0; i < num_procs; i++ ){
        int num_correct;
        if(read(fdread[i][0], &num_correct, sizeof(int)) == -1){
            perror("read");
            exit(1);
        }
