...
int main(int argc, char *argv[]) {
  ...
  dataset_train = load_dataset(argv[1]);
  dataset_test = load_dataset(argv[2]);
  DTNode *dtnode = build_dec_tree(dataset_train);
  Image *img;
  for(int i = 0; i < dataset_test->num_items; i++){
      img = &(dataset_test->images[i]); // go through all the images in the testing dataset
      if(dataset_test->labels[i] == dec_tree_classify(dtnode, img)){ //see if the image match with its label
        total_correct += 1;
      }
  }
