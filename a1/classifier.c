...
/**
 * Compilation command
 *    gcc -Wall -std=c99 -lm -O3 -o classifier classifier.c knn.c
 *
...
int main(int argc, char *argv[]) {  
    ...
    for (int i = 0; i < num_test_files; i++){
        int predict_result = knn_predict(test_dataset[i], K, training_dataset, training_labels, num_training_files);
        if(predict_result == test_labels[i]){
            num_correct += 1;
        }
